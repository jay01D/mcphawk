...
The dashboard lives at <http://localhost:4800> while mcphawk is running. It's a single-page React app served by the same process that runs the proxy.

## Layout

- **Header** — the mcphawk name, a live/offline badge for the WebSocket, and a filter input.
...
- **offline** — server gone or unreachable. Refresh the page if it stays offline after mcphawk restarts.

## Where you'll get stuck

- **Nothing shows up.** The wrapped command never received any traffic. Check that your MCP client is pointing at `npx mcphawk -- <command>` and not the bare command.
- **Rows appear but the WebSocket says offline.** mcphawk is logging to SQLite but the dashboard server died. Check the terminal where you ran mcphawk.
